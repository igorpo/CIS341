...
let cfg_of_stream (code:stream) : Ll.cfg * (Ll.gid * Ll.gdecl) list  =
    let gs, einsns, insns, term_opt, blks = List.fold_left
      (fun (gs, einsns, insns, term_opt, blks) e ->
        ...
        | T t  -> (gs, einsns, [], Some (Ll.Parsing.gensym "tmn", t), blks)
...
and cmp_rty ct : Ast.rty -> Ll.ty = function
  ...
  | Ast.RFun (ts, t) -> Fun (cmp_fty ct (ts, t))
...
(* Helper function that allocates an oat structure on the 
...
let rec cmp_exp (tc : TypeCtxt.t) (c:Ctxt.t) (exp:Ast.exp node) : Ll.ty * Ll.operand * stream =
  ...
  (* Code that compiles struct expressions.
  ...
  | Ast.CStruct (id, l) ->
    ...
    let add_elt s f =
      let field_type = cmp_ty tc @@ TypeCtxt.lookup_field id f.cfname tc in
      let index = TypeCtxt.index_of_field id f.cfname tc in
      let elt_op, elt_code = cmp_exp_as tc c f.cfinit field_type in
      let ind = gensym "ind" in
      s >@ elt_code >@ lift
        [ ind, Gep(struct_ty, struct_op, [Const 0L; i64_op_of_int index])
        ; "", Store(field_type, elt_op, Id ind) ]
    in
...
let cmp_function_ctxt (tc : TypeCtxt.t) (c:Ctxt.t) (p:Ast.prog) : Ctxt.t =
    List.fold_left (fun c -> function
      | Ast.Gfdecl { elt={ rtyp; name; args } } ->
         let ft = TRef (RFun (List.map fst args, rtyp)) in
         Ctxt.add c name (cmp_ty tc ft, Gid name)
      | _ -> c
    ) c p 
...
let rec cmp_gexp c (tc : TypeCtxt.t) (e:Ast.exp node) : Ll.gdecl * (Ll.gid * Ll.gdecl) list =
  ...
  | CStruct (id, cs) ->
    ...
    let elts, gs = 
      List.fold_right
        (fun f (elts, gs) ->
          let gd, gs' = cmp_gexp c tc (List.find (fun x -> x.cfname = f.fname) cs).cfinit in
