...
and print_cfield_aux l fmt c =
  pp_open_box fmt 0; 
  pp_print_string fmt c.cfname; 
  pp_print_string fmt "="; print_exp_aux l fmt c.cfinit;
  pp_close_box fmt ();
...
let print_field fmt f =
  let pps = pp_print_string fmt in
  pp_open_hbox fmt ();
  print_ty_aux fmt f.ftyp; pps " "; pps f.fname;
  pp_close_box fmt ()
...
and ml_string_of_field (c : cfield) : string =
  sp "%s = %s;" (ml_string_of_id c.cfname) (ml_string_of_exp c.cfinit)
...
let rec ml_string_of_fdecl_aux (f:fdecl) : string =
  ...
  (ml_string_of_ret_ty f.rtyp) (ml_string_of_id f.name)
...
let ml_string_of_field f : string =
  sp "{ fname = %s; typ = %s }"
     (ml_string_of_id f.fname) (ml_string_of_ty f.ftyp)
...
let rec eq_fdecl_aux (f1:fdecl) (f2:fdecl) : bool =
  eq_ret_ty f1.rtyp f2.rtyp && eq_id f1.name f2.name &&
